...
export default function Header() {
  ...
  return (
    <nav className="sticky top-0 grid grid-flow-col left-0 h-[70px] z-50 bg-gray-50 dark:bg-gray-800 outline outline-1 outline-slate-300/40 dark:outline-slate-900 w-full">
      ...
      <div className="grid gap-[120px] grid-flow-col absolute self-center justify-self-end mr-[30px]">
        <ModeSwitcher />
        {user ? (
          <div className="grid">
            <UserAvatar className="overflow-hidden grid w-[50px] h-[50px] rounded-full" />
          </div>
        ) : (
          <></>
        )}
